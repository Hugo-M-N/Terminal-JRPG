...
public class TextMenus {
	...
	public static String Menu(String[] options) {
		...
		try {
			while(true) {
				...
				String insert = "";
				String tmp ="";
				window+="╠" + "═".repeat(Config.getTERMINAL_WIDTH()) + "╣\n";
				
				window+="║ ";
				for(int i=0; i<options.length;i++) {
					tmp+=("  ╔"+"═".repeat(options[i].length()+2)+"╗  ");
				}
				while(tmp.length()<Config.getTERMINAL_WIDTH()-1) {
					tmp += " ";
				}
				window+= tmp;
				window+="║\n";
	...
	public static void OptionsMenu() throws IOException {		
		String[] options = {"Width & Height", "Go Back"};
		int sel = 0;
		...
		try {
			while(true) {
                ...
                case "ENTER":
                	...
                	int oldW=Config.getTERMINAL_WIDTH(), oldH=Config.getTERMINAL_HEIGTH();
                	while(sub) {
                		InputHelper.clearScreen();
                		for(String s : ScreenBuffer.getScreenBuffer()) System.out.println(s);
                		String  subMenu= InputHelper.READER.readBinding(InputHelper.KEYMAP);
                		
                        switch (subMenu) {
                        case "UP":
                            Config.setTERMINAL_HEIGTH(Config.getTERMINAL_HEIGTH()+1);
                            break;
                        case "DOWN":
                        	Config.setTERMINAL_HEIGTH(Config.getTERMINAL_HEIGTH()-1);
                        case "RIGHT":
                        	Config.setTERMINAL_WIDTH(Config.getTERMINAL_WIDTH()+1);
                        	break;
                        case "LEFT":
                        	Config.setTERMINAL_WIDTH(Config.getTERMINAL_WIDTH()-1);
                        	break;
                        case "ENTER":
                        	...
                        	break;
                        case "ESCAPE":
                        	Config.setTERMINAL_WIDTH(oldW);
                        	Config.setTERMINAL_HEIGTH(oldH);
                        	break;
                    	default:
                    		break;
                        }
	...
	public static void Inventory(ArrayList<Entity> Allies) {
	    ...
	    String tmp = "";
	    int itemWidth = (2 * Config.getTERMINAL_WIDTH() / 3) - 1;
	    int infoWidth = (Config.getTERMINAL_WIDTH() / 3) - 2;
	    ...
	    try {
	        while (menu) {
	            ...
	            for (int heigth = 0; heigth <= Config.getTERMINAL_HEIGTH(); heigth++) {
	                tmp = "";
	                if (heigth == Config.getTERMINAL_HEIGTH() - 5) {
	                    tmp += borders[4] + " ".repeat(itemWidth) + borders[5] + borders[2].repeat(infoWidth) + borders[6];
	                } else if (heigth >= Config.getTERMINAL_HEIGTH() - 4 && heigth <= Config.getTERMINAL_HEIGTH() - 1) {
	                    tmp += borders[4] + " ".repeat(itemWidth) + borders[4];
	                    int optionIndex = heigth - (Config.getTERMINAL_HEIGTH() - 4);
	                    ...
	                    tmp += borders[4];
	                } else if (heigth == Config.getTERMINAL_HEIGTH()) {
	                    ...
	                    if (Items.size() > heigth) {
	                        ...
	                        String itemInfo = itemName + " -X"+ Items.get(heigth).getAMOUNT();
	                        tmp += itemInfo + " ".repeat(itemWidth - itemInfo.length());
	                    } else {
	                ...
	                switch (key) {
	                    ...
	                    case "ENTER":
	                    	...
	                    	if(Items.get(itemSel) instanceof Usable) {
	                    		Entity target = TextMenus.SelectTarget(Allies);
	...
	public static void shopMenu(List<Item> Items, ArrayList<Entity> Allies) {
		String[] borders = {"╔","╦","═","╗","║","╠","╣","╚","╩","╝","╬"};
		ArrayList<String> result = new ArrayList<>();
		String tmp = "";
		int itemWidth = (2 * Config.getTERMINAL_WIDTH() / 3) - 1;
		...
		try {
			while (menu) {
				...
				result.add(borders[0] + borders[2].repeat(itemWidth) + borders[1] + borders[2].repeat(infoWidth) + borders[3]);
				result.add(borders[4] + "Shop" + " ".repeat(itemWidth - "Shop".length()) + borders[4] + "Item info" + " ".repeat(infoWidth - "Item info".length()) + borders[4]);
				result.add(borders[5] + borders[2].repeat(itemWidth) + borders[10] + borders[2].repeat(infoWidth) + borders[6]);
				
				for (int heigth = 0; heigth <= Config.getTERMINAL_HEIGTH(); heigth++) {
					tmp = "";
					if (heigth >= Config.getTERMINAL_HEIGTH() - 4 && heigth <= Config.getTERMINAL_HEIGTH() - 1) {
						tmp += borders[4] + " ".repeat(itemWidth) + borders[4];
						...
						if (Items.size() > heigth) {
							String itemName = Items.get(heigth).getNAME();
							if (heigth == itemSel) itemName = "-> " + itemName;
							String itemInfo = itemName + " -X"+ Items.get(heigth).getAMOUNT() + " - " + Items.get(heigth).getPRICE()+"g"; 
							tmp += itemInfo + " ".repeat(itemWidth - itemInfo.length());
						} else {
							tmp += " ".repeat(itemWidth);
						}
